      ...
      <div className="min-h-screen bg-cream dark:bg-background pattern-dots pb-[calc(100px+env(safe-area-inset-bottom,16px))] relative">
        ...
        <main className="page-shell__main pt-28 sm:pt-32 max-w-4xl mx-auto space-y-6 px-4 relative" style={{ paddingTop: 'calc(88px + var(--sat, 0px) + 1.5rem)' }}>
          ...
          {(isAdmin || developerMode) && (
            <section className="bg-gradient-to-br from-indigo-500/10 via-purple-500/5 to-white dark:from-indigo-950/30 dark:via-purple-950/15 dark:to-card border border-indigo-500/30 rounded-3xl p-6 shadow-sm space-y-4">
              <div className="flex items-center justify-between border-b border-indigo-500/20 pb-3">
                <h3 className="text-[14px] font-bold uppercase tracking-[0.08em] text-indigo-700 dark:text-indigo-300 flex items-center gap-2">
                  <Cpu className="w-4.5 h-4.5 text-indigo-600 dark:text-indigo-400" />
                  {language === 'bm' ? 'Integrasi Ejen AI & MCP Server (Khas Admin)' : 'AI Agent & MCP Integration (Admin Only)'}
                </h3>
                <span className="text-[11px] font-extrabold px-2.5 py-0.5 rounded-full bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 border border-indigo-500/30">
                  MCP 2024-11-05
                </span>
              </div>

              <p className="text-xs text-stone dark:text-stone/80 leading-relaxed">
                {language === 'bm'
                  ? 'Aplikasi ini sedia disambungkan ke Ejen AI luaran (Custom GPTs, Gemini Extensions, Claude Desktop MCP, atau Zapier AI) melalui Model Context Protocol (MCP) dan OpenAPI 3.0.'
                  : 'This app is ready to connect with external AI Agents (Custom GPTs, Gemini Extensions, Claude Desktop MCP, or Zapier AI) via Model Context Protocol (MCP) and OpenAPI 3.0.'}
              </p>

              <div className="space-y-3 font-mono text-xs">
                {/* Endpoint 1: MCP SSE Stream URL */}
                <div className="p-3 bg-white/80 dark:bg-stone-900/60 rounded-xl border border-indigo-500/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                  <div>
                    <span className="font-sans font-bold text-deep-forest dark:text-white block text-xs">
                  ...
                  <Button
                    ...
                    className="rounded-lg h-8 px-3 text-[11px] border-indigo-500/30 text-indigo-600 dark:text-indigo-300 hover:bg-indigo-50"
                  >
                    {language === 'bm' ? 'Salin Pautan' : 'Copy URL'}
                  </Button>
                </div>

                {/* Endpoint 2: OpenAPI Specification URL */}
                <div className="p-3 bg-white/80 dark:bg-stone-900/60 rounded-xl border border-indigo-500/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                  <div>
                    <span className="font-sans font-bold text-deep-forest dark:text-white block text-xs">
              ...
              {/* Quick Available Tools */}
              <div className="pt-1">
                <span className="font-sans font-bold text-xs text-deep-forest dark:text-white block mb-1.5">
                  {language === 'bm' ? 'Alat MCP Yang Disokong (AI Tools):' : 'Supported MCP AI Tools:'}
                </span>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px] font-sans">
                  <div className="p-2.5 rounded-xl bg-white/60 dark:bg-stone-900/40 border border-border">
                    <span className="font-bold font-mono text-indigo-600 dark:text-indigo-400 block">get_menu_items</span>
                    <span className="text-stone dark:text-stone/70 text-[10px]">Carian menu katering & harga halal</span>
                  </div>
                  <div className="p-2.5 rounded-xl bg-white/60 dark:bg-stone-900/40 border border-border">
                    <span className="font-bold font-mono text-indigo-600 dark:text-indigo-400 block">calculate_catering_estimate</span>
                    <span className="text-stone dark:text-stone/70 text-[10px]">Kira anggaran kos mengikut jumlah pax</span>
                  </div>
                  <div className="p-2.5 rounded-xl bg-white/60 dark:bg-stone-900/40 border border-border">
                    <span className="font-bold font-mono text-indigo-600 dark:text-indigo-400 block">check_order_status</span>
                    <span className="text-stone dark:text-stone/70 text-[10px]">Semak status rujukan tempahan</span>
                  </div>
                  <div className="p-2.5 rounded-xl bg-white/60 dark:bg-stone-900/40 border border-border">
                    <span className="font-bold font-mono text-indigo-600 dark:text-indigo-400 block">submit_catering_inquiry</span>
                    <span className="text-stone dark:text-stone/70 text-[10px]">Hantar tempahan katering baharu</span>
                  </div>
                </div>
              </div>
            </section>
