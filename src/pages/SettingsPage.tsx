...
function SettingsSection({
  ...
  return (
    <div className="space-y-2">
      {title && (
        <h2 className="px-1 text-xs font-bold uppercase tracking-[0.12em] text-stone-500 dark:text-stone-400">
          {title}
        </h2>
      )}
      ...
      <div className="min-h-screen bg-cream dark:bg-background pb-28 sm:pb-32 relative">
        ...
        <header className="glass-header fixed top-0 left-0 right-0 z-50 pt-[var(--sat)]">
          <div className="max-w-3xl mx-auto flex items-center justify-between px-4 sm:px-6 min-h-[58px] sm:min-h-[64px]">
             ...
             * Semantic identity header & logo touch target navigating to /home.
            ...
            >
              ...
              <div>
                ...
                <span className="block text-xs font-bold uppercase tracking-[0.16em] text-[var(--color-sunshine-cta)] leading-none mt-0.5">
                  Pak Usop
                </span>
              </div>
            </button>
        ...
        >
              ...
              <div className="flex items-center justify-between">
                ...
                <SegmentedControl
                  ...
                  onValueChange={async (val) => {
                    await triggerLightImpact();
                    setAdminSubTab(val as 'appearance' | 'updates' | 'diagnostics');
                  }}
                  size="sm"
                >
                  <SegmentedControlItem value="appearance">
                    <Palette className="w-3.5 h-3.5 inline mr-1" />
                    UI
                  </SegmentedControlItem>
                  <SegmentedControlItem value="updates">
                    <Radio className="w-3.5 h-3.5 inline mr-1" />
                    Updates
                  </SegmentedControlItem>
                  <SegmentedControlItem value="diagnostics">
                ...
                <div className="space-y-4 pt-2 border-t border-amber-500/20">
                    ...
                    <div className="flex items-center gap-1.5">
                      {[
                        ...
                        <ColorSwatch
                          ...
                          value={preset.hex}
                          title={preset.name}
                          selected={customMainColor.toLowerCase() === preset.hex.toLowerCase()}
                          onSelect={async (hex) => {
                            await triggerLightImpact();
                            setCustomMainColor(hex);
                          }}
                        />
                      ))}
                  ...
                  <div className="flex justify-end pt-1">
                    <Button
                      ...
                      className="text-xs font-bold text-amber-700 dark:text-amber-300 hover:underline flex items-center gap-1 p-0 h-auto"
                    >
            ...
            {isAdmin ? (
              <>
                <div className="p-4 sm:p-5 flex items-center justify-between gap-3 border-b border-stone-200/70 dark:border-white/5 bg-stone-50/50 dark:bg-white/[0.02]">
                  ...
                  <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 shrink-0">
          ...
          <SettingsSection title={language === 'bm' ? 'Pilihan Paparan' : 'Preferences'}>
            ...
            <SettingsRow
              ...
              action={
                <SegmentedControl
                  value={language}
                  onValueChange={(val) => handleLanguageChange(val as 'en' | 'bm')}
                >
                  <SegmentedControlItem value="en">
                    English
                  </SegmentedControlItem>
                  <SegmentedControlItem value="bm">
                    Melayu
                  </SegmentedControlItem>
                </SegmentedControl>
              }
          ...
          <SettingsSection title={language === 'bm' ? 'Maklum Balas Sentuhan & Bunyi' : 'Haptics & Audio'}>
            ...
            <SettingsRow
              ...
              title={language === 'bm' ? 'Kesan Bunyi Sentuhan Mechanical "Thock"' : 'Mechanical "Thock" Touch Sounds'}
              isLast={true}
              action={
                <div className="flex items-center gap-2">
                  {soundEffectsEnabled && (
                    <Button
                      variant="secondary"
                      size="sm"
                      onClick={() => playClickSound('force', 'thock')}
                      className="px-2.5 py-1 rounded-lg bg-violet-500/10 hover:bg-violet-500/20 border border-violet-500/20 text-violet-700 dark:text-violet-300 font-bold text-xs transition-colors flex items-center gap-1 focus-visible:ring-2 focus-visible:ring-violet-500 h-auto"
                      title={language === 'bm' ? 'Uji Bunyi Thock' : 'Test Thock Sound'}
                      aria-label={language === 'bm' ? 'Uji Bunyi Thock Mekanikal' : 'Test Mechanical Thock Sound'}
                    >
                      <Play className="w-3 h-3 fill-current" />
                      <span>{language === 'bm' ? 'Uji Thock' : 'Test Thock'}</span>
                    </Button>
                  )}
                  <Switch
                    ...
                    aria-label="Toggle mechanical thock touch sounds"
                  />
                </div>
              }
            />
                  ...
                  <div className="flex items-center gap-1.5">
                    {[
                      ...
                      <ColorSwatch
                        ...
                        title={preset.name}
                        size="sm"
                        selected={statusBarColor.toLowerCase() === preset.hex.toLowerCase()}
                        onSelect={async (hex) => {
                          await triggerLightImpact();
              ...
              {[
                ...
                <div key={idx} className="p-3 rounded-xl bg-stone-100 dark:bg-stone-800/80 border border-stone-200/60 dark:border-white/5 space-y-0.5">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-mono font-semibold px-2 py-0.5 rounded-md bg-stone-200 dark:bg-stone-700 text-stone-700 dark:text-stone-300">
                      {item.license}
                    </span>
                  </div>
