...
export function HungryButton({ text, onHungryClick, className, ...props }: HungryButtonProps) {
  ...
  return (
    <button
      onClick={handleClick}
      className={cn(
        "relative overflow-hidden w-full sm:w-auto px-8 py-4 bg-[var(--color-sunshine-cta)] text-white font-black rounded-2xl shadow-lg hover:brightness-110 active:scale-[0.98] transition-all flex items-center justify-center gap-3 cursor-pointer group/hungry",
        className
      )}
      {...props}
    >
